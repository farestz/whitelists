...
const (
	...
	categoryRuListFile = "category-ru.list"
)

// Loyalsoldier publishes a compiled geosite.dat where the CATEGORY-RU tag holds
// the fully-resolved geosite:category-ru domain set. We decode it and emit a
// Shadowrocket-format rule-set (category-ru.list) for clients that can't read
// v2ray .dat (Shadowrocket, Surge, etc.).
var (
	loyalsoldierGeositeURL = "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat"
	categoryRuTag          = "category-ru"
)
...
func runBuild() {
	...
	// Build category-ru.list (Shadowrocket rule-set) from Loyalsoldier geosite.
	if err := buildCategoryRuList(); err != nil {
		fmt.Fprintf(os.Stderr, "category-ru.list: %v\n", err)
		os.Exit(1)
	}
}

// buildCategoryRuList downloads Loyalsoldier's compiled geosite.dat, extracts
// the CATEGORY-RU tag, and writes a Shadowrocket-format rule-set. Domain types
// map to: domain → DOMAIN-SUFFIX, full → DOMAIN, plain → DOMAIN-KEYWORD. Regex
// entries are skipped — Shadowrocket rule-sets have no domain-regexp rule type.
func buildCategoryRuList() error {
	...
	entries, err := decodeTaggedDomains(dat, categoryRuTag)
	if err != nil {
		return err
	}
	if len(entries) == 0 {
		return fmt.Errorf("tag %q not found or empty in %s", categoryRuTag, dat)
	}

	var b strings.Builder
	b.WriteString("# geosite:category-ru -> Shadowrocket rule-set\n")
	b.WriteString("# source: Loyalsoldier/v2ray-rules-dat geosite.dat (tag CATEGORY-RU)\n")
	b.WriteString("# generated by farestz/whitelists — do not edit by hand\n")
	...
	data := []byte(b.String())
	if err := os.WriteFile(categoryRuListFile, data, 0644); err != nil {
		return err
	}
	fmt.Printf("category-ru.list: %d rules (%d regex skipped), %d bytes\n", rules, skipped, len(data))
	writeChecksum(categoryRuListFile, data)
	return nil
}
