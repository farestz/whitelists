...
- `geosite:category-ip-geo-detect` — copied verbatim from Loyalsoldier's `geosite.dat` (the same download `buildGeositeRuleSets()` uses) and embedded as an extra tag. Exists so Happ can **block** it on-device (`BlockSites: ["geosite:category-ip-geo-detect", …]`) against the single `Geositeurl` .dat — moving the drop off the proxy server (`block` outbound) onto the client, no round-trip. Driven by `geositeBlockTags` in `main.go`; to add/remove a block category, edit that slice (and the Happ `BlockSites` in `routing/whitelists.json`). The DIRECT tag is untouched, so friends consuming only `geosite:direct` are unaffected, and the stable `whitedomains.dat` URL is unchanged.

> [!warning] No big tags here — iOS 50 MB NE limit
> `category-ads-all` (~165k domains) was tried here and **broke Happ**: xray runs inside an iOS Network Extension capped at ~50 MB RAM, and building the in-memory matcher for 165k domains exceeds it (`VPN extension memory limit(50mb) exceeded` → `NEAgentErrorDomain` error 1 → tunnel won't start). The `.dat` file itself was valid (verified with `v2ray test`) — it's a runtime memory ceiling, not a format bug. Ads stay blocked **server-side** (in whitelist mode, non-whitelisted ad domains go to PROXY where the server's `block` outbound drops them). Only keep **small** geosite tags in `geositeBlockTags`. Shadowrocket is unaffected — it consumes `category-ads-all` as a separate streamed `RULE-SET`, not via this `.dat`.
